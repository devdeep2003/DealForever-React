...
export default function CategoryDetail() {
  ...
  const meta = categoryMeta[activeCategory] ?? {
    ...
    subtitle: 'Explore our complete range of products',
  };

  const filtered = products.filter((p) => {
    const productSlug = p.category?.toLowerCase().replace(/\s+/g, '-');
    const matchCat = activeCategory === '' || productSlug === activeCategory;
    const matchSearch = p.name.toLowerCase().includes(search.toLowerCase());
    return matchCat && matchSearch;
  });

  const handleTabClick = (tabSlug: string) => {
    setActiveCategory(tabSlug);
    if (tabSlug === '') {
      navigate('/categories');
    } else {
      navigate(`/categories/${tabSlug}`);
    }
  };
